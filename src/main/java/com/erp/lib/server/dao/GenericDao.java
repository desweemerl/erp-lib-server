package com.erp.lib.server.dao;

import com.erp.lib.server.database.DBTable;
import com.erp.lib.server.database.DBTableComparator;
import com.erp.lib.server.database.DBTables;
...
import com.erp.lib.server.utils.IntegerUtil;
...
public class GenericDao {
    ...
    protected List getRecords(Class tableClass, String condition, Object[] values, String order, boolean orderAsc)
            throws Exception {
        DBTable table = DBTables.getDBTable(tableClass);
        return table.select(getConnection(), condition, values, order, orderAsc);
    }

    protected List getRecords(Class tableClass, String condition, Object[] values, String[] orders, boolean[] ordersAsc)
            throws Exception {
        DBTable table = DBTables.getDBTable(tableClass);
    ...
    protected Map insertRecord(String tableName, Map<String, Object> mapInput, String[] fieldsReturned)
            throws Exception {
        ...
        String SQL = "INSERT INTO " + tableName + " (" + StringUtil.join(keys, ",")
                + ") VALUES (" + StringUtil.repeat("?", ",", size) + ")";
    ...
    protected void saveSubRecords(Class tableClass, Map<String, Integer> conditions, List<GenericTable> genericTables)
            throws Exception {
        if (genericTables == null) {
            return;
        }
        ...
        for (GenericTable existingTable : existingTables) {
            ...
            for (GenericTable genericTableBkp : genericTablesBkp) {
                if (genericTableBkp.getId() != null && IntegerUtil.isEqual(existingTable.getId(), genericTableBkp.getId())) {
        ...
        genericTablesBkp.forEach((genericTableBkp) -> {
    ...
    protected void saveSubRecords(Class tableClass, Map<String, Integer> conditions, List<GenericTable> genericTables,
            DBTableComparator dbTableComparator) throws Exception {
        if (genericTables == null) {
            return;
        }
        ...
        genericTablesBkp.forEach((genericTableBkp) -> tablesForInsert.add(genericTableBkp));
    ...
    protected void saveSubRecords(Class conditionTableClass, Map<String, Integer> conditions, Class tableClass,
            List<GenericTable> genericTables, DBTableComparator dbTableComparator) throws Exception {
        if (genericTables == null) {
            return;
        }
