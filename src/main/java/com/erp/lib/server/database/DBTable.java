...
import com.erp.lib.server.json.request.ListRequest;
import com.erp.lib.server.json.response.ListResponse;
import com.erp.lib.server.table.GenericTable;
...
public class DBTable {
    ...
    private final Class genericTableClass;
    private final String name;
    private final Map<String, DBField> dbFields = new HashMap();
    private final Map<String, String> dbFieldNameClass = new HashMap();
    private final List<String> fieldsNames = new ArrayList();
    private final Pattern namedCriteriaPattern = Pattern.compile("\\%\\([a-zA-Z]{1,}\\)s");
    ...
    public boolean hasFieldName(String fieldName) {
        return this.fieldsNames.stream().anyMatch((name) -> (name.compareTo(fieldName) == 0));
    }
    ...
    public List select(Connection connection, String condition, Object[] values, String order, boolean orderAsc)
            throws Exception {
        StringBuilder SQL = new StringBuilder("SELECT * FROM ").append(name);
        boolean withCriteria = false;
        ...
        if (order != null) {
            ...
            if (orderFieldName == null) {
                throw new Exception("ERROR on select arguments: order field name '" + order + "' is not registered");
            }
    ...
    public List select(Connection connection, String condition, Object[] values, String[] orders, boolean[] ordersAsc)
            throws Exception {
        int n, l;
        ...
        if ((l = orders.length) != ordersAsc.length) {
            throw new Exception("ERROR on select arguments: orders and ordersAsc must be of the same size");
        }
        ...
        for (n = 0; n < l; n++) {
            if (orders[n] != null) {
                ...
                if (orderFieldName == null) {
                    throw new Exception(
                            "ERROR on select arguments: order field name '" + orders[n] + "' is not registered");
                }
    ...
    public ListResponse select(Connection connection, String condition, ListRequest request) throws Exception {
        ...
        try (PreparedStatement ps1 = connection.prepareStatement(countSQL.toString())) {
            ...
            if (count > 0) {
                boolean withOrder = false;
                Integer offset = 0, page = request.getPage(),
                ...
                if (order != null) {
                    ...
                    if (orderFieldName == null) {
                        throw new Exception(
                                "ERROR on select arguments: order field name '" + order + "' is not registered");
                    }
                    ...
                    for (n = 0; n < l; n++) {
                        if ((order = orders.get(n)) != null) {
                            ...
                            orderFieldName = this.dbFieldNameClass.get(order);
                            if (orderFieldName == null) {
                                throw new Exception("ERROR on select arguments: order field name '" + order
                                        + "' is not registered");
                            }
                ...
                querySQL.append(" OFFSET ").append(String.valueOf(offset)).append(" LIMIT ")
    ...
    public GenericTable update(Connection connection, GenericTable genericTable) throws Exception {
        ...
        String SQL = "UPDATE " + name + " SET " + StringUtil.join(parameters, ",")
                + " WHERE id = ?" + " RETURNING " + StringUtil.join(fieldsNames, ",");
    ...
    public int[] batchUpdate(Connection connection, List<GenericTable> genericTables) throws Exception {
        ...
        String SQL = "UPDATE " + name + " SET "
                + StringUtil.join(parameters, ",") + " WHERE id = ?";
    ...
    public GenericTable insert(Connection connection, GenericTable genericTable) throws Exception {
        ...
        String SQL = "INSERT INTO " + name + " ("
                + StringUtil.join(parameters, ",") + ") VALUES ("
                + StringUtil.repeat("?", ",", parameters.size()) + ")" + " RETURNING "
                + StringUtil.join(fieldsNames, ",");
    ...
    public int[] batchInsert(Connection connection, List<GenericTable> genericTables) throws Exception {
        ...
        String SQL = "INSERT INTO " + name + " ("
                + StringUtil.join(parameters, ",") + ") VALUES ("
                + StringUtil.repeat("?", ",", parameters.size()) + ")";
